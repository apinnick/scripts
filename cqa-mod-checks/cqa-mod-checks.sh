...
echo -e "This report describes modularization issues found in '$ASSEMBLIES' and '$MODULES'.\n" >> $OUTPUT
...
# Generated by Gemini. START check for include directives on consecutive lines
CHECK="'include::' directives on consecutive lines"
echo -e "$CHECK:\n" >> $OUTPUT
echo -e "- $CHECK" >> $SUMMARY
# Define the function to check for sequential includes.
check_includes() {
  local file="$1"
  local last_line_was_include=0
  ...
  while IFS= read -r line; do
    # Skip lines that contain "ifdef", "ifeval", or "endif".
    if [[ "$line" =~ (ifdef|ifeval|endif) ]]; then
      last_line_was_include=0
      continue
    fi

    # Check if the current line starts with "include::".
    if [[ "$line" =~ ^include:: ]]; then
      if [[ "$last_line_was_include" -eq 1 ]]; then
        # If a sequence is found, print the filename and the line.
        echo "- $file"
        break # Exit the loop early once a sequence is found.
      fi
      last_line_was_include=1
    else
      last_line_was_include=0
    fi
  done < "$file"
